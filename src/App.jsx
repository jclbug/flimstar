...
function Filmmakers({ title, maker }) {
  return (
...
function OtherRatings({ rating }) {
  return (
    <div className="flex gap-[10px] bg-[#52796f] font-bold text-(--heading) text-lg pt-[10px] pb-[10px] pl-[24px] pr-[24px] rounded-[10px]">
      <p>{rating.Source}:</p>
      <p>{rating.Value}</p>
    </div>
  );
...
function MovieSubData({ heading = "", data, emoji = "" }) {
  return (
    <p className="text-lg text-(--heading)">
...
function LoadingMsg({ message }) {
  return <p className="text-center font-bold text-2xl mt-[48px]">{message}</p>;
}
